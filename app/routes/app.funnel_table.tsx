  ...
  try {
    ...
    for (const product of funnel.products) {
      ...
      const metafields = allMetafields.filter(
        (metafield: any) => metafield.node.namespace === "product_data"
      );
      ...
      if (metafields.length > 0) {
        ...
        console.warn(
          `No metafields found for product ${product.shopifyId} in namespace "product_data".`
        );
      }
    }
...
//         query {
//           product(id: "${product.shopifyId}") {
//             metafields(namespace: "product_data", first: 10) {
//               edges {
//                 node {
//                   id
//                 }
...
//       const metafieldResponse: any = await admin.graphql(metafieldQuery);
//       const metafields = metafieldResponse?.data?.product?.metafields?.edges || [];
//       console.log("metafields", metafields)
//       for (const metafield of metafields) {
//         const metafieldId = metafield?.node?.id;
...
//           if (deleteResponse?.data?.metafieldDelete?.userErrors?.length) {
//             console.error(
//               `Errors while deleting metafield for product ${product.shopifyId}:`,
//               deleteResponse.data.metafieldDelete.userErrors
//             );
//           } else {
//             console.log(
//               `Metafield deleted successfully for product ${product.shopifyId}`
//             );
//           }
//         } else {
//           console.warn(
//             `Metafield not found for product ${product.shopifyId} in namespace "product_data".`
//           );
//         }
//       }
//     }
