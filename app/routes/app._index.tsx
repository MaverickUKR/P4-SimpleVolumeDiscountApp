...
function OffersTable() {
  ...
  const [selectedFunnel, setSelectedFunnel] = useState<Offer | null>(null);
  ...
  const handleDelete = (offer: Offer) => {
    setSelectedFunnel(offer); // Устанавливаем весь объект Offer
    setActiveModal(true);
  };

  const confirmDelete = () => {
    if (selectedFunnel) {
      fetcher.submit({ funnelId: selectedFunnel.id }, { method: "post" });
    }
    setActiveModal(false);
  };

  const rowMarkup = displayedOffers.map((offer, index) => (
    <IndexTable.Row id={offer.id} key={offer.id} position={index}>
      ...
      <IndexTable.Cell>{offer.creationDate}</IndexTable.Cell>
      <IndexTable.Cell>{offer.products}</IndexTable.Cell>
      <IndexTable.Cell>
        <Badge tone="success">{offer.status}</Badge>
      </IndexTable.Cell>
      <IndexTable.Cell>
        <Button
          icon={DeleteIcon}
          variant="primary"
          tone="critical"
          onClick={() => handleDelete(offer)} // Передаем весь объект Offer
        />
      </IndexTable.Cell>
    </IndexTable.Row>
  ));
